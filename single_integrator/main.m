...
Delta = 0.1;
eta = 0.133; %eta_star found by solving the gradient ascent problem for the given value of Delta 
...
for traj_itr = 1:traj_num
    ...
    for t = t0:h:T-h % this loop is to find u(t) and x(t) at each time step t => x(t+h) = x(t) + f(x(t)).h + G_u.u(t).h + Sigma*dw
        ...
        S_tau_all = arrayfun(@simulateMC, eps_t_all_1, eps_t_all_2, xt(1), xt(2), f_xt(1), f_xt(2), t, h, T, b, s, xR, xS, yR, yS, xP, xQ, yP, yQ, eta, k, c, d, Delta); %an array that stores S(tau) of each sample path starting at time t and state xt
        ...
        ut = ((s/c)*numer)/(sqrt(h)*denom); %the agent control input
...
%         end
        ...
        xt = xt + f_xt*h + c*ut*h + s*eps*sqrt(h);
