...
Delta = 0.1;
eta = 0.769; %eta_star found by solving the gradient ascent problem for the given value of Delta 
...
for traj_itr = 1:traj_num
    ...
    for t = t0:h:T-h % this loop is to find u(t) and x(t) at each time step t => x(t+h) = x(t) + f(x(t)).h + G_u.u(t).h + Sigma*dw
            ...
            S_tau_all = arrayfun(@simulateMC, eps_t_all_1, eps_t_all_2, px_t, py_t, v_t, theta_t, phi_t, fpx_t, fpy_t, fv_t, ftheta_t, fphi_t, t, h, T, b, s, xR1, xS1, yR1, yS1, xR2, xS2, yR2, yS2, xP, xQ, yP, yQ, eta, k1, k2, k3, k4, d, L, Delta); %an array that stores S(tau) of each sample path starting at time t and state xt
