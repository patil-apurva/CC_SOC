function [S_tau_all, risk_tau_all] = risk_analysis_simulateMC(eps_t_all_1, eps_t_all_2, px_t_prime, py_t_prime, v_t_prime, theta_t_prime, phi_t_prime, fpx_t_prime, fpy_t_prime, fv_t_prime, ftheta_t_prime, fphi_t_prime, t0, h, T, b, s, xR1, xS1, yR1, yS1, xR2, xS2, yR2, yS2, xP, xQ, yP, yQ, eta, k1, k2, k3, k4, d, L, Delta)
    ...
    safe_flag_tau = 1; %is the sample path tau safe?
    risk_tau = 0; %the risk of a sample path tau
    
    for t_prime = t0:h:T-h % this loop is to compute S(tau_i) and risk(tau_i)
        ...
        if (((px_t_prime>=xR1) && (px_t_prime<=xS1) && (py_t_prime>=yR1) && (py_t_prime<=yS1)) || ((px_t_prime>=xR2) && (px_t_prime<=xS2) && (py_t_prime>=yR2) && (py_t_prime<=yS2)) || ((px_t_prime<=xP) || (px_t_prime>=xQ) || (py_t_prime<=yP) || (py_t_prime>=yQ)))%if yes means t_prime=t_exit
            ...
            risk_tau = 1; %the risk of a sample path tau
            safe_flag_tau = 0;
            break; %end this tau 
        end
    ...
    S_tau = S_tau - eta*Delta;

    S_tau_all = S_tau;
    risk_tau_all = risk_tau;